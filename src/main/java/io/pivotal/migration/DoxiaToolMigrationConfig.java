...
/**
 * Configuration for migration of DOXIATOOLS Jira.
 */
@Configuration
@Import(CommonApacheMavenMigrationConfig.class)
@ConditionalOnProperty(name = "jira.projectId", havingValue = "DOXIATOOLS")
public class DoxiaToolMigrationConfig {
    @Primary
    @Bean
    ...
    public JiraIssueFilter jiraIssueFilter(@Value("${jira.component}") String componentName) {
        return new CompositeJiraIssueFilter(new DoxiaToolMigrationConfig.JiraIssueComponentFiler(componentName));
    }
